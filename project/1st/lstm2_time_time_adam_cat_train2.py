...
for i in range(len(aX)):
	XX = np.zeros((1, len(aX[i])), dtype=np.int32)
...
for i in range(0, len(aY)):
	code = 0
	YY = np.zeros((1, len(aY[i]), vocab_size), dtype=np.bool)
...
for i in range(0, len(taY)):
	code = 0
	tYY = np.zeros((1, len(taY[i]), vocab_size), dtype=np.bool)
...
print('Training ...')
fResult = open('lstm2_time_time_adam_cat_train.txt', 'a+')
...
for i in range(0, EPOCHS):
	...
	for j in range(len(aX)):
		loss, acc = model.train_on_batch(X[j], Y[j], accuracy=True)
		index += 1
		if (index == BATCH_SIZE) or (j == len(aX)-1):

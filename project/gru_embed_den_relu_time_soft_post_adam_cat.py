...
model.add(GRU(EMBED_SIZE, HIDDEN_SIZE))
...
print('Training ...')
fResult = open('gru_embed_den_relu_time_soft_post_adam_cat.txt', 'w')
...
fPredict = open('gru_embed_den_relu_time_soft_post_adam_cat_pred.txt', 'w')
...
class LossHistory(Callback):
	def on_epoch_end(self, epoch, logs={}):
		fPredict = open('gru_embed_den_relu_time_soft_post_adam_cat_pred.txt', 'a+')
		...
		fResult = open('gru_embed_den_relu_time_soft_post_adam_cat.txt', 'a+')
...
history = LossHistory()
model.fit(aX, Y, batch_size=BATCH_SIZE, nb_epoch=EPOCHS, validation_data=(taX, tY, twY), verbose=1, show_accuracy=True, callbacks=[history], sample_weight=wY)
...
model.save_weights('gru_embed_den_relu_time_soft_post_adam_cat.hdf5', overwrite=True)

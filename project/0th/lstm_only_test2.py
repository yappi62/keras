...
def vectorize(data):
	...
	return pad_sequences(X, maxlen=ques_maxlen), pad_sequences(Y, maxlen=ques_maxlen)
...
X, aY = vectorize(train)
tX, taY = vectorize(test)
del annIdsA, annsA, train, test
...
model.add(LSTM(EMBED_SIZE, HIDDEN_SIZE, return_sequences=True))

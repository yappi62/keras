...
Y = np.zeros((len(aY), vocab_size, ques_maxlen), dtype=np.bool)
	...
	for j in range(0, ques_maxlen):
		if start == False:
			if aY[i, j] != 0:
				start = True
		if start == True:
			Y[i, aY[i, j], ques_maxlen-1-j] = 1

tX = np.zeros((len(taX), ques_maxlen, vocab_size), dtype=np.bool)
tY = np.zeros((len(taY), vocab_size, ques_maxlen), dtype=np.bool)
	...
	for j in range(0, ques_maxlen):
		if start == False:
			if taY[i, j] != 0:
				start = True
		if start == True:
			tY[i, taY[i, j], ques_maxlen-1-j] = 1
...
fResult.write('Average learning time = %d:%d:%.2f'%(avgHour, avgMin, avgSec))
